...
export default function OrganizerEventsPage() {
  const events = listOrganizerEvents();
    ...
    <main className="min-h-dvh px-10 py-10">
      ...
      <div className="overflow-hidden rounded-xl border border-border">
        ...
        {events.map((event) => {
          ...
          const editable = !isPastEvent(event.date);
          ...
          return (
            ...
            >
              <span className="font-semibold">{event.catalogItem.title}</span>
              <span className="text-text-dim">
                {formatEventDate(event.date)}
              </span>
              <span className="text-text-dim">{event.venue}</span>
